...
public class MegaFFDWarpEditor : MegaWarpEditor
{
	...
	public override void DrawSceneGUI()
	{
		...
		if ( ffd.DisplayGizmo )
		{
			...
			//if ( context && context.DrawGizmos )
			{
				...
				//Quaternion rot = Quaternion.Euler(Vector3.zero);	//ffd.gizmoRot);
				...
				for ( int z = 0; z < gs; z++ )
				{
					for ( int y = 0; y < gs; y++ )
					{
						for ( int x = 0; x < gs; x++ )
						{
							...
							if ( handles )
							{
								ttp = PositionHandle(tp, Quaternion.identity, handleSize, ffd.GizCol1.a);
	...
	{
		Handles.color = Color.yellow;	//ffd.gizCol1;	//Color.red;
